...
class Player:
    def __init__(self):
        ...
        self.caminho_personagem = "assets/graphics/personagens"

        self.sprites_direcao = {
            "frente": pygame.image.load(f"{self.caminho_personagem}/Jailson de frente.png").convert_alpha(),
            "costas": pygame.image.load(f"{self.caminho_personagem}/Jailson de costas.png").convert_alpha(),
            "direita": pygame.image.load(f"{self.caminho_personagem}/Jailson direita.png").convert_alpha(),
            "esquerda": pygame.image.load(f"{self.caminho_personagem}/Jailson esquerda.png").convert_alpha(),
            "frente_direita": pygame.image.load(
                f"{self.caminho_personagem}/Jailson diagonal frente direita.png").convert_alpha(),
            "frente_esquerda": pygame.image.load(
                f"{self.caminho_personagem}/Jailson diagonal frente esquerda.png").convert_alpha(),
            "costas_direita": pygame.image.load(
                f"{self.caminho_personagem}/Jailson diagonal costas direita.png").convert_alpha(),
            "costas_esquerda": pygame.image.load(
                f"{self.caminho_personagem}/Jailson diagonal costas esquerda.png").convert_alpha(),
        }


        for direcao, imagem in self.sprites_direcao.items():
            self.sprites_direcao[direcao] = pygame.transform.smoothscale(imagem, (49, 63))
    ...
    def atualizar_direcao(self, dx, dy):
        if dx == 0 and dy == 0:
            ...
            self.direcao_atual = "frente_direita"
        elif dx < 0 and dy > 0:
            self.direcao_atual = "frente_esquerda"
        elif dx > 0 and dy < 0:
            self.direcao_atual = "costas_direita"
        elif dx < 0 and dy < 0:
            self.direcao_atual = "costas_esquerda"
        elif dx > 0:
    ...
    def draw(self, superficie):
        sprite_atual = self.sprites_direcao[self.direcao_atual]
        rect_sprite = sprite_atual.get_rect(center=self.rect.center)
        superficie.blit(sprite_atual, rect_sprite)

        # Se estiver carregando lixo, desenha uma bolinha verde em cima dele indicando o inventário cheio
        if self.carregando_lixo:
            pygame.draw.circle(superficie, (200, 0, 0), (self.rect.centerx, self.rect.top - 20), 6)
